...
if action == "capabilities":
    result = {"redengine_version":"1.1.0-final","redatamx_version":"1.1.3","selection":True,"number":True,"inherited_define":True,"freq":True,"cmpcode":False,"table_view":True}
elif action == "inspect":
    result = {
      "entities": [
    ...
    plan = req["plan"]
    # Legacy runtime plans must not contain @cmpcode-derived geography dimensions.
    assert not any(field.startswith("XPROV") or field.startswith("XDPTO") or field.startswith("XFRAC") or field.startswith("XRADIO") for field in plan["dimension_fields"])
    bases = {
    ...
    for base in bases[plan["entity"]]:
      ...
      for field in plan["dimension_fields"]:
        value = base.get(field, field + "-" + str(base[plan["own_id"]]))
        row[field] = value
        row[masks[field]] = 0
      row["count"] = 1
      rows.append(row)
...
def test_cli_extract_uses_selected_radio_as_scope_on_legacy_runtime(tmp_path, capsys):
    ...
    assert payload["geography_key_mode"] == "selection-code-fallback"

    persons = read_parquet_rows(output / "persona.parquet")
    assert persons[0]["XRADIO"] == "061471101"
    ...
    assert manifest["scope_source"] == "selection-code-fallback"
    assert runtime["redengine_version"] == "1.1.0-final"
    assert runtime["redatamx_version"] == "1.1.3"
    assert runtime["cmpcode"] is False
